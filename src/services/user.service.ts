...
class UserService {
  ...
  public async partialUpdateUser(
    ...
    if (dto.name && dto.name.length < 3) {
      throw new ApiError("Name should be minimum 3 symbols", 400);
    }
    if (dto.email && !dto.email.includes("@")) {
      throw new ApiError("Email is invalid", 400);
    }
    if (dto.password && dto.password.length < 8) {
      throw new ApiError("Password should be minimum 8 symbols", 400);
    }

    return await userRepository.partialUpdateById(userId, dto);
  }
}
